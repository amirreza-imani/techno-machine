      ...
      <section className="bg-[#f7f7f5] py-16 md:py-20">
        <Container>
          <div className="rounded-3xl border border-gray-200 bg-white px-6 py-16 text-center shadow-sm">
            <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-brand-black text-xl font-black text-brand-gold">
              TM
            </div>

            <h2 className="mt-6 text-2xl font-black text-brand-black">
              پروژه‌ای برای نمایش وجود ندارد
            </h2>

            <p className="mx-auto mt-3 max-w-lg text-sm leading-8 text-gray-500">
              اطلاعات پروژه پس از ثبت و انتشار در سیستم نمایش داده خواهد شد.
            </p>
          </div>
        </Container>
      </section>
              ...
              <div className="border-b border-gray-100 px-6 py-8 md:px-10 md:py-10">
                <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
                  <div className="max-w-3xl">
                    ...
                    <h2 className="mt-4 text-3xl font-black leading-[1.35] text-brand-black md:text-4xl">
                      {project.title}
                    </h2>
                  </div>

                  <span className="inline-flex w-fit rounded-full border border-brand-gold/20 bg-brand-gold/5 px-4 py-2 text-xs font-bold text-brand-gold">
                    تکنو ماشین
                  </span>
                </div>

                {project.shortDescription && (
                  <p className="mt-6 max-w-3xl text-sm leading-8 text-gray-600 md:text-base">
                    {project.shortDescription}
                  </p>
                )}
              </div>
                  ...
                  {gallery.slice(0, 2).map((image, index) => {
                    ...
                    return (
                      <div
                        ...
                        className="group relative aspect-video w-full overflow-hidden rounded-2xl bg-brand-charcoal"
                      >
                        <Image
                          src={imageUrl}
                          alt={
                            image.alternativeText ||
                            `${project.title} - تصویر ${index + 2}`
                          }
                          fill
