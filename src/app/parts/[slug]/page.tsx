				...
				<section className='bg-brand-black py-16 text-white md:py-24'>
					<Container>
						<div className='max-w-4xl'>
							<Link href='/parts' className='mb-8 inline-flex items-center text-sm font-semibold text-white/50 transition-colors hover:text-brand-gold'>
								<span className='ml-2'>→</span>
								بازگشت به قطعات
							</Link>

							<span className='mb-4 block text-sm font-bold text-brand-gold'>قطعات ماشین‌آلات</span>

							<h1 className='text-4xl font-black leading-tight md:text-6xl'>{part.title}</h1>

							<p className='mt-6 max-w-3xl text-base leading-8 text-gray-300 md:text-lg'>{part.shortDescription}</p>
						</div>
					</Container>
				</section>
						...
						<div className='grid gap-10 lg:grid-cols-[1.05fr_0.95fr] lg:items-start'>
							...
							<div className='overflow-hidden rounded-2xl border border-border-theme light:white dark:bg-brand-charcoal'>
								<div className='relative aspect-[4/3]'>
									{imageUrl ? (
										<Image src={imageUrl} alt={part.image?.alternativeText || `تصویر ${part.title}`} fill sizes='(max-width: 1024px) 100vw, 55vw' className='object-cover' />
									) : (
										<div className='flex h-full items-center justify-center'>
											<span className='text-7xl font-black text-brand-gold/20'>TM</span>
										</div>
									)}
								</div>
							</div>
							...
							<div className='rounded-2xl border border-border-theme bg-surface p-7 shadow-sm md:p-9'>
								<span className='text-sm font-bold text-brand-gold'>مشخصات قطعه</span>

								<h2 className='mt-3 text-2xl font-black text-foreground md:text-3xl'>{part.title}</h2>

								<div className='mt-7 h-px bg-border-theme' />

								<div className='mt-7'>
									<h3 className='text-base font-black text-foreground'>توضیحات</h3>

									<p className='mt-4 text-sm leading-8 text-muted md:text-base'>{part.shortDescription}</p>
								</div>
				...
				<section className='bg-[#f3f3f0] py-16 text-brand-black transition-colors duration-300 dark:bg-brand-black dark:text-white md:py-20'>
					<Container>
						<div className='flex flex-col items-start justify-between gap-6 md:flex-row md:items-center'>
							<div>
								<span className='text-sm font-bold text-brand-gold'>نیاز به این قطعه دارید؟</span>

								<h2 className='mt-2 text-2xl font-black md:text-3xl'>برای استعلام و تأمین قطعه با ما در تماس باشید.</h2>
							</div>
